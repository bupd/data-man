...
def is_event_match(artist, event_date, event_venue):
    ...
    if artist in artist_event_map:
        logging.info(f"checking for artist: {artist}")
        print(f"checking for artist: {artist}")
        for event in artist_event_map[artist]:
            # Check if the event date and venue match
            logging.info(f"artist got event date: {event_date}, correct date: {event[2]} ")
            print(f"artist got event date: {event_date}, correct date: {event[2]} ")
            logging.info(f"artist got event venue: {event_venue}, correct venue: {event[1]} ")
...
try:
    ...
    def search_artist(artist):
        ...
        try:
            ...
            try:
                ...
                print(f"Fans shopping for '{artist}': {fans_count}")
            except Exception:
                fans_count = ""
                logging.warning(f"Fans count not found for '{artist}'.")
                print(f"Fans count not found for '{artist}'.")
                #raise

            return fans_count
        except Exception as e:
            logging.error(f"search_artist() Error during artist search: {e}")
            print(f"search_artist() Error during artist search: {e}")
            #raise
        ...
        try:
            ...
            while True:
                ...
                for element in elements:
                    ...
                    for attempt in range(2):
                        try:
                            ...
                            if match:
                                logging.info(f"trying this element: {match}")
                                print(f"trying this element: {match}")
                                ...
                                if is_event_match(artist, formatted_date, venue):
                                    ...
                                    logging.info("No match for this event")
                                ...
                                try:
                                    fans_count_element = driver.find_element(By.XPATH, "//android.widget.TextView[contains(@text, 'fans shopping for these tickets')]")
                                    fans_count_text = fans_count_element.text
                                    logging.info(f'fans_count_text: {fans_count_text}')
                                    print(f'fans_count_text: {fans_count_text}')
                                ...
                                print(f"Collected event: {event_name}, {formatted_date}, {time_}, {venue}")
                                driver.back()
                                time.sleep(2)
                            else:
                                if attempt == 2:
                ...
                try:
                    ...
                    print("show_more button clicked.")
                    time.sleep(5)
                except Exception:
                    scroll_down(driver)
        except Exception as e:
    ...
    if spreadsheet_data:
        ...
        for data in spreadsheet_data:
            ...
            # date_object = datetime.strptime(curr_date, "%Y-%m-%dT%H:%M:%S.%fZ")
            # formatted_date = date_object.strftime("%m/%d/%Y")
            logging.info("\nPrinting input row data")
            print("\nPrinting input row data")
            logging.info(f"Artist: {data['Artist']}, Date: {date}, Venue: {data['Venue']}")
    ...
    for artist in artist_event_map:
        try:
            logging.info(f'c_event_count: {c_event_count}')
            print(f'c_event_count: {c_event_count}')
